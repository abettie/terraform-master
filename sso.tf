...
# adminグループにAdministratorAccessを割り当て(sandbox-01アカウント)
resource "aws_ssoadmin_account_assignment" "admin_assignment_sandbox_01" {
  instance_arn       = tolist(data.aws_ssoadmin_instances.main.arns)[0]
  permission_set_arn = aws_ssoadmin_permission_set.administrator_access.arn
  principal_id       = aws_identitystore_group.admin.group_id
  principal_type     = "GROUP"
  target_id          = aws_organizations_account.sandbox_01.id
  target_type        = "AWS_ACCOUNT"
}

# adminグループにAdministratorAccessを割り当て(sandbox-02アカウント)
resource "aws_ssoadmin_account_assignment" "admin_assignment_sandbox_02" {
  instance_arn       = tolist(data.aws_ssoadmin_instances.main.arns)[0]
  permission_set_arn = aws_ssoadmin_permission_set.administrator_access.arn
  principal_id       = aws_identitystore_group.admin.group_id
  principal_type     = "GROUP"
  target_id          = aws_organizations_account.sandbox_02.id
  target_type        = "AWS_ACCOUNT"
}
